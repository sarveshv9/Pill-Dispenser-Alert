...
class Alarm(db.Model):
    ...
    start_date = db.Column(db.String(10), nullable=False)  # Store in YYYY-MM-DD format
    end_date = db.Column(db.String(10), nullable=False)    # Store in YYYY-MM-DD format
...
def add_alarm():
    ...
    image = request.form.get('image')  # If you're handling image uploads

    if medicine and time and start_date and end_date:
        new_alarm = Alarm(medicine=medicine, time=time, start_date=start_date, end_date=end_date, image=image)
