...
def database():
    ...
    return render_template('database.html', alarms=alarms)
...
@app.route('/mark_completed/<int:alarm_id>', methods=['POST'])
def mark_completed(alarm_id):
    alarm = Alarm.query.get(alarm_id)
    if alarm:
        alarm.completed = True
        db.session.commit()
        return jsonify({'message': 'Alarm marked as completed'}), 200
    return jsonify({'error': 'Alarm not found'}), 404
...
def upcoming():
    ...
    return render_template('upcoming.html', alarms=upcoming_alarms)
