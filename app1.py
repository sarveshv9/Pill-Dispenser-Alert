...
class Alarm(db.Model):
    ...
    start_date = db.Column(db.String(10), nullable=True)  # Add start date
    end_date = db.Column(db.String(10), nullable=True)    # Add end date
...
def add_pill_alarm():
    ...
    start_date = request.form.get('start_date')  # Fetch from form
    end_date = request.form.get('end_date')
    
    start_date = datetime.strptime(start_date, "%Y-%m-%d") if start_date else None
    end_date = datetime.strptime(end_date, "%Y-%m-%d") if end_date else None
      
    new_alarm = Alarm(medicine=f"Pill - {name}", time=time, start_date=start_date, end_date=end_date)
...
def mark_alarm_done():
    alarm = Alarm.query.filter_by(completed=False).first()
    if alarm:
        alarm.completed = True
        db.session.commit()
